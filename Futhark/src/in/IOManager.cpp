...
bool IOManager::readTextFileToStringVector(const std::string& FILE_PATH, std::vector<std::string>& fileContents) {
	// Open the file
	...
	fileContents.clear();
	std::string line;
	while (std::getline(file, line)) { fileContents.push_back(line + "\n"); }
	// Close file
	file.close();
	return true;
}
