...
void GameWorld::create(fk::Tools& tools) {
	...
	text += "Right Click = Set primary objective for current squad\n";
	...
	// Player
	Player::Def pd;
	pd.body = tools.textures.get("Selector.png", 1);
	...
	pd.bd.bd.position.x = 0;
	pd.bd.bd.position.y = 0;
	playerPtr = new Player(*spriteBatchPtr, *textBatchPtr, cam, world, pd);
...
void GameWorld::update(fk::Tools& tools) {
	...
	if (tools.ui.getKeyInfo(fk::Key::CTRL_L).downFrames || tools.ui.getKeyInfo(fk::Key::CTRL_R).downFrames) {
		if (tools.ui.getKeyInfo(fk::Key::Z).downFrames == 1) {
			Dummy::Def dd;
			dd.body = tools.textures.get("Selector.png", 1);
			dd.bd.sightRadius = 1;
		...
		if (tools.ui.getKeyInfo(fk::Key::NUM1).downFrames == 1) {
			Equipment::Def ed;
			ed.body = tools.textures.get("Test.png", 1);
			ed.cursorAccesory = tools.textures.get("Selector.png", 1);
			ed.projectileSpawner.body = tools.textures.get("Circle.png", 1);
